...
import { BookOpen, Video, GraduationCap, Repeat, Plus, Trash2, ExternalLink, CheckCircle, Circle, Edit2, X } from 'lucide-react';
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold flex items-center gap-2 text-slate-300">
                    ...
                    <span>أهداف التطوير الذاتي</span>
                </h2>
                <button
                    onClick={() => {
                        resetForm();
                    ...
                    className="bg-primary-500 hover:bg-primary-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm"
                >
                    <Plus size={18} />
                    <span>هدف جديد</span>
                </button>
            ...
            {showForm && (
                <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 mb-8 animate-in fade-in slide-in-from-top-4 relative">
                    <button
                        onClick={resetForm}
                    ...
                    <h3 className="text-lg font-bold mb-4 text-white">
                        {editingId ? 'تعديل الهدف' : 'إضافة هدف جديد'}
                    </h3>
                    ...
                    >
                        {editingId ? 'حفظ التعديلات' : 'حفظ الهدف'}
                    </button>
                </div>
            )}
            ...
            <div className="space-y-4">
                {goals.length === 0 ? (
                    <div className="text-center py-12 text-slate-500 bg-slate-800/50 rounded-xl border border-dashed border-slate-700">
                        ...
                        <p>لا توجد أهداف حالياً. ابدأ بإضافة هدف جديد لتطوير ذاتك!</p>
                    </div>
                ) : (
                    goals.map(goal => (
                        <div key={goal.id} className={`bg-slate-800 rounded-xl p-4 border border-slate-700 flex items-center justify-between group ${goal.status === 'completed' ? 'opacity-60' : ''}`}>
                            ...
                            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => handleEdit(goal)} className="text-slate-600 hover:text-blue-400 p-2">
                                    <Edit2 size={18} />
                                </button>
